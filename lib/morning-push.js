...
function buildMorningMessage({ today, events = [], bills = [], todos = [], weather = null, heading = '今日小叮嚀' }) {
...
async function createMorningMessage({ notion, config, now = new Date(), calendarClient, weatherData, dayOffset = 0 }) {
  const today = addDays(taipeiDate(now), dayOffset);
  ...
  const text = buildMorningMessage({
    today, events, bills, todos, weather,
