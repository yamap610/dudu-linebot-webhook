...
function formatBill(bill, today) {
  ...
  return `▪️ 💳 ${bill.name}（${label}）${price}`;
}
...
function calendarSections(events) {
  ...
  for (const event of events) {
    ...
    activities.push(`▪️ ${item.icon} ${item.short}｜${eventTimeRange(event)} ${displayTitle}`);
  }
...
function buildMorningMessage({ today, events = [], bills = [], weather = null }) {
...
async function createMorningMessage({ notion, config, now = new Date(), calendarClient }) {
  ...
  return buildMorningMessage({ today, events, bills, weather });
}
...
module.exports = {
  taipeiDate, getUpcomingBills, calendarSections, getWeather, buildMorningMessage, createMorningMessage, pushLine,
};
