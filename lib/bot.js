...
function calendarEventsMessage(label, events) {
  ...
  const lines = events.slice(0, 30).map((event) => {
    ...
    return `${eventDate(event).replace(/-/g, '/')}｜${startTime}｜${owner}\n　${truncate(title, 50)}`;
  });
...
function todoListMessage(type, items) {
  ...
  rows.unshift({
    ...
    contents: [
      { type: 'text', text: '優先級說明', size: 'xs', weight: 'bold', color: '#6B7280' },
      { type: 'text', text: '🔥 急　　⭐ 中　　🌿 緩', size: 'xs', color: '#6B7280', margin: 'xs' },
    ],
  });
...
async function handleCommand(command, notion, config) {
  ...
  if (command.action === 'calendar_add') {
    ...
    const when = detailed
      ? (allDay ? `${command.start}｜整天` : `${command.start.replace('T', ' ')}–${command.end.replace('T', ' ')}`)
      : `${command.date} ${command.time || '整天'}`;
