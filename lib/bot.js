...
async function getTodoItems(notion, config, type) {
  ...
  // Notion 的資料庫查詢不會回傳頁面內容，因此只檢查 LINE 當頁會顯示的前 10 項。
  for (const item of items.slice(0, 10)) {
    item.hasChecklist = (await getChecklist(notion, item.id).catch(() => [])).length > 0;
  }
  return items;
}
...
function addDescriptionMessage(type, priority, name) {
  const meta = TODO_TYPES[type];
  const prefix = type === 'buy' ? '新增待買說明' : '新增待辦說明';
  return textMessage(`${meta.emoji} ${name}\n優先級：${priority}\n\n要加上一段說明嗎？`, [
    quickPostback('⌨️ 填寫說明', 'action=noop', { inputOption: 'openKeyboard', fillInText: `${prefix}｜${priority}｜${name}｜` }),
    quickPostback('略過，直接新增', `action=add_create&type=${type}&priority=${priority}&name=${encodeURIComponent(name)}`),
    quickPostback('取消', `action=add_menu&type=${type}`),
  ]);
}

async function addTodo(notion, config, type, priority, name, note = '') {
  const meta = TODO_TYPES[type];
  const properties = {
...
async function handleCommand(command, notion, config) {
  ...
  if (command.action === 'add' && TODO_TYPES[command.type]) {
    ...
    return [addDescriptionMessage(command.type, command.priority, name)];
  }
  if (command.action === 'add_create' && TODO_TYPES[command.type] && ['急', '中', '緩'].includes(command.priority)) {
    ...
    if (!name) return [addMenuMessage(command.type)];
    await addTodo(notion, config, command.type, command.priority, name, note);
    const meta = TODO_TYPES[command.type];
    return [textMessage(`${meta.emoji} 已新增：${name}\n優先級：${command.priority}${note ? `\n說明：${note}` : ''}`, [
