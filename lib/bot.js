  ...
  const rows = shown.length ? shown.map((item) => {
    const contents = [
      { type: 'box', layout: 'vertical', flex: 4, contents: [
        ...
        { type: 'text', text: `${billStatus(item)}${money(item.price) ? ` ${money(item.price)}` : ''}${item.amountUpdatedDate ? `（${item.amountUpdatedDate.slice(5).replace('-', '/')}更新）` : ''}${item.cancelling ? '・準備取消' : ''}`, size: 'xs', color: daysFromToday(item.dueDate || '9999-12-31') < 0 ? '#C2413B' : '#6B7280', wrap: true },
      ] },
...
module.exports = {
  ...
  billsMessage, updateAmountMenuMessage, paidAmountMessage, calendarMenuMessage, calendarAddMenuMessage,
