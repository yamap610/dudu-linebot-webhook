  ...
  const rows = shown.length ? shown.map((item) => {
    const contents = [
      { type: 'box', layout: 'vertical', flex: 4, contents: [
        ...
        { type: 'text', text: `${billStatus(item)}${money(item.price) ? `・${money(item.price)}` : ''}${item.amountUpdatedDate ? `・金額更新 ${item.amountUpdatedDate.replace(/-/g, '/')}` : ''}${item.cancelling ? '・準備取消' : ''}`, size: 'xs', color: daysFromToday(item.dueDate || '9999-12-31') < 0 ? '#C2413B' : '#6B7280', wrap: true },
      ] },
