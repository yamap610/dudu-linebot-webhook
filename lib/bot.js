...
const { getExpiryItems, formatExpiryBlock } = require('./expiry');
...
async function handleCommand(command, notion, config) {
  ...
  if (command.action === 'expiry') {
    ...
    const heading = command.overdueOnly
      ? '【 效期提醒｜已過期 】'
      : '【 效期提醒｜已過期／3 天內 】';
    const block = formatExpiryBlock(items, heading);
    return [textMessage(block || '目前沒有需要注意的效期品項。')];
  }
...
module.exports = {
  ...
  getExpiryItems, formatExpiryBlock, handleCommand, markBillPaid, updateBillAmount,
};
