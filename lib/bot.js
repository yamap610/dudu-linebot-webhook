const DB_DEFAULTS = {
  ...
  expiry: 'f40a3915e858492daa9a941ddccea34d',
};
const { createCalendarClient, addDays, weekRange, eventDate, eventTimeRange } = require('./calendar');
const { getExpiryItems, expiryCountdown, formatExpiryBlock } = require('./expiry');
...
const CALENDAR_LOCATION_COLORS = { '6': '回桃園', '1': '回宜蘭' };
...
function getConfig(env = process.env) {
  return {
    ...
    expiryDbId: env.EXPIRY_DB_ID || DB_DEFAULTS.expiry,
    expiryNotionUrl: env.EXPIRY_NOTION_URL || `https://app.notion.com/p/${env.EXPIRY_DB_ID || DB_DEFAULTS.expiry}`,
...
function createNotionClient(config, fetchImpl = fetch) {
  ...
  return {
    queryAll,
    getDatabase: (databaseId) => request(`/databases/${databaseId}`),
    getPage: (pageId) => request(`/pages/${pageId}`),
    getBlockChildren: (blockId) => request(`/blocks/${blockId}/children?page_size=100`),
    appendBlockChildren: (blockId, children) => request(`/blocks/${blockId}/children`, {
      method: 'PATCH', body: JSON.stringify({ children }),
    }),
    updateBlock: (blockId, body) => request(`/blocks/${blockId}`, {
...
async function getChecklist(notion, parentId, depth = 0) {
  if (typeof notion.getBlockChildren !== 'function' || depth > 3) return [];
  const data = await notion.getBlockChildren(normalizeId(parentId));
  const items = [];
  for (const block of data.results || []) {
    if (block.type === 'to_do') {
      const text = blockRichText(block);
      if (text) items.push({
        id: normalizeId(block.id), text, checked: Boolean(block.to_do?.checked),
      });
    }
    if (block.has_children) {
      items.push(...await getChecklist(notion, block.id, depth + 1));
    }
  }
  return items;
}
...
function calendarLocation(labelName, colorId, title) {
  return CALENDAR_LOCATIONS[labelName] || CALENDAR_LOCATION_COLORS[String(colorId)]
    || ['回桃園', '回宜蘭'].find((place) => String(title || '').startsWith(place));
}

function calendarLabelText(labelName, colorId, title) {
  const location = calendarLocation(labelName, colorId, title);
  if (location) return `📍 ${location.replace('回', '')}`;
...
function calendarEventsMessage(label, events) {
  ...
  const locationEntries = events.map((event) => {
    const title = event.summary || '';
    const location = calendarLocation(event.eventLabelName, event.colorId, title);
    if (!location || !title.startsWith(location)) return null;
    const date = eventDate(event).slice(5).replace('-', '/');
    return { date, location, key: `${date}|${location}` };
  }).filter(Boolean);
  const uniqueLocationEntries = [...new Map(locationEntries.map((entry) => [entry.key, entry])).values()];
  const locationKeys = new Set(uniqueLocationEntries.map((entry) => entry.key));
  const showDates = label.includes('週');
  const sortedEvents = [...events].sort((a, b) => {
    const aTime = eventTimeRange(a).split('–')[0];
    const bTime = eventTimeRange(b).split('–')[0];
    const aKey = `${eventDate(a)}T${aTime === '整天' ? '00:00' : aTime}`;
    const bKey = `${eventDate(b)}T${bTime === '整天' ? '00:00' : bTime}`;
    return aKey.localeCompare(bKey);
  });
  const lines = sortedEvents.slice(0, 30).map((event) => {
    const title = event.summary || '（未命名）';
    const location = calendarLocation(event.eventLabelName, event.colorId, title);
    const date = eventDate(event).slice(5).replace('-', '/');
    if (location && title.startsWith(location)) return `${showDates ? `${date}｜` : ''}📍 ${location}`;
    const definition = calendarLabelDefinition(event.eventLabelName, '', title);
    const owner = location && locationKeys.has(`${date}|${location}`)
      ? (definition ? `${definition.emoji} ${definition.short}` : '🗓️ 其他')
      : calendarLabelText(event.eventLabelName, event.colorId, title);
    const startTime = eventTimeRange(event).split('–')[0];
    const prefix = showDates
      ? `${date}${startTime === '整天' ? '' : ` ${startTime}`}｜`
      : (startTime === '整天' ? '' : `${startTime}｜`);
    return `${prefix}${owner}｜${truncate(title, 50)}`;
  });
  return textMessage(`📅 ${label}\n\n${lines.join('\n')}${sortedEvents.length > 30 ? `\n\n另有 ${sortedEvents.length - 30} 筆未顯示` : ''}`, mainQuickReplies());
}
...
async function getTodoItems(notion, config, type) {
  ...
  const items = pages.map((page) => ({
    ...
    dueDate: String(page.properties['預定作業日期']?.date?.start || '').slice(0, 10),
    category: selectName(page.properties['類別']),
    note: todoNote(page.properties),
    notionUrl: safeHttpUrl(page.url),
    hasChecklist: page.properties['內含明細']?.checkbox === true,
  }));
  return items;
}
...
function todoListMessage(type, items) {
  ...
  const rows = shown.length ? shown.map((item, index) => ({
    type: 'box', layout: 'vertical', spacing: 'xs', margin: index === 0 ? 'xl' : 'md',
    contents: [
      ...
      { type: 'box', layout: 'horizontal', justifyContent: 'flex-end', alignItems: 'center', spacing: 'xl', margin: 'sm', contents: [
        ...(item.hasChecklist && item.notionUrl ? [{
          type: 'text', text: '開啟清單 ↗', size: 'xs', color: '#526D82', weight: 'bold', flex: 0,
          action: { type: 'uri', label: '開啟清單', uri: item.notionUrl },
        }] : []),
        {
          type: 'text', text: type === 'buy' ? '已買到' : '完成', size: 'xs', color: '#526D82', weight: 'bold', flex: 0,
          action: postbackAction(type === 'buy' ? '已買到' : '完成', `action=complete_confirm&type=${type}&id=${item.id}`),
        },
        {
          type: 'text', text: '編輯提醒', size: 'xs', color: '#526D82', weight: 'bold', flex: 0,
          action: postbackAction('編輯提醒', `action=edit_reminder&type=${type}&id=${item.id}`),
        },
      ] },
      ...(index < shown.length - 1 ? [{ type: 'separator', margin: 'md', color: '#E5E7EB' }] : []),
    ],
...
async function getTodoDetail(notion, pageId) {
  ...
  const note = todoNote(props);
  const checklist = await getChecklist(notion, pageId);
  return {
    id: normalizeId(page.id || pageId),
    name: richText(props['項目名稱']) || '（無標題）',
    priority: selectName(props['優先級']) || '中',
    dueDate: String(props['預定作業日期']?.date?.start || '').slice(0, 10),
...
function checklistMessage(type, item, requestedPage = 1) {
  const meta = TODO_TYPES[type];
  if (!item) return textMessage('找不到這個項目，可能已被刪除。', [quickPostback(`返回${meta.label}`, `action=list&type=${type}`)]);
  if (!item.checklist?.length) return textMessage(`「${item.name}」目前沒有內容清單。`, [
    quickPostback(`返回${meta.label}`, `action=list&type=${type}`),
  ]);
  const pageSize = 8;
  const pageCount = Math.ceil(item.checklist.length / pageSize);
  const page = Math.min(Math.max(Number(requestedPage) || 1, 1), pageCount);
  const shown = item.checklist.slice((page - 1) * pageSize, page * pageSize);
  const rows = shown.map((entry, index) => ({
    type: 'box', layout: 'horizontal', alignItems: 'center', spacing: 'sm',
    margin: index === 0 ? 'lg' : 'md',
    contents: [
      { type: 'text', text: `${entry.checked ? '☑' : '☐'} ${truncate(entry.text, 55)}`, wrap: true, size: 'sm', color: entry.checked ? '#9CA3AF' : '#374151', flex: 5 },
      { type: 'text', text: entry.checked ? '取消' : '勾選', size: 'xs', color: '#526D82', weight: 'bold', align: 'end', flex: 1,
        action: postbackAction(entry.checked ? '取消' : '勾選', `action=checklist_toggle&type=${type}&id=${item.id}&block=${entry.id}&checked=${entry.checked ? '0' : '1'}&page=${page}`) },
    ],
  }));
  const footer = [];
  if (page > 1) footer.push(button('上一頁', `action=checklist&type=${type}&id=${item.id}&page=${page - 1}`));
  if (page < pageCount) footer.push(button('下一頁', `action=checklist&type=${type}&id=${item.id}&page=${page + 1}`));
  footer.push(button('重新整理清單', `action=checklist&type=${type}&id=${item.id}&page=${page}`));
  footer.push(button(type === 'buy' ? '全部買齊' : '整項完成', `action=complete_confirm&type=${type}&id=${item.id}`, 'primary'));
  footer.push(button(`返回${meta.label}`, `action=list&type=${type}`));
  const checkedCount = item.checklist.filter((entry) => entry.checked).length;
  return flexMessage(`${meta.label}內容清單`, baseBubble(
    `${meta.emoji} ${truncate(item.name, 60)}`,
    `${checkedCount}/${item.checklist.length} 項完成${pageCount > 1 ? `｜第 ${page}/${pageCount} 頁` : ''}`,
    [
      ...(item.note ? [{ type: 'text', text: `說明：${truncate(item.note, 180)}`, wrap: true, size: 'xs', color: '#6B7280', margin: 'md' }] : []),
      { type: 'text', text: '點選後會直接同步，勾完請按「重新整理清單」查看。', wrap: true, size: 'xs', color: '#9A6E76', margin: 'md' },
      ...rows,
    ],
    footer,
  ));
...
async function toggleChecklistItem(notion, pageId, blockId, checked) {
  const checklist = await getChecklist(notion, pageId);
  const item = checklist.find((entry) => entry.id === normalizeId(blockId));
  if (!item) throw new Error('找不到這個清單項目，請重新開啟清單。');
  await notion.updateBlock(item.id, { to_do: { checked: Boolean(checked) } });
}
...
function editReminderMessage(type, item) {
  const meta = TODO_TYPES[type];
  if (!item) return textMessage('找不到這筆項目，可能已被刪除。', [quickPostback(`返回${meta.label}`, `action=list&type=${type}`)]);
  const base = `type=${type}&id=${item.id}`;
  return textMessage(
    `🔔 編輯提醒｜${item.name}\n目前優先級：${item.priority}\n預定作業日期：${item.dueDate || '未設定'}\n\n可分別修改優先級或日期。`,
    [
      ...['急', '中', '緩'].map((priority) => quickPostback(
        `${priority === '急' ? '🔥' : priority === '中' ? '⭐' : '🌿'} 改為${priority}`,
        `action=edit_priority&${base}&priority=${priority}`,
      )),
      quickPostback('📅 更改日期', `action=edit_date&${base}`, { type: 'datetimepicker', mode: 'date', initial: item.dueDate || taipeiToday() }),
      quickPostback('清除日期', `action=edit_date&${base}&clear=1`),
      quickPostback(`返回${meta.label}`, `action=list&type=${type}`),
    ],
  );
}

async function updateTodoReminder(notion, pageId, changes) {
  if (!validPageId(pageId)) throw new Error('無效的項目編號');
  const properties = {};
  if (['急', '中', '緩'].includes(changes.priority)) properties['優先級'] = { select: { name: changes.priority } };
  if (changes.clearDate) properties['預定作業日期'] = { date: null };
  else if (/^\d{4}-\d{2}-\d{2}$/.test(changes.date || '')) properties['預定作業日期'] = { date: { start: changes.date } };
  if (!Object.keys(properties).length) throw new Error('沒有可更新的提醒設定');
  await notion.updatePage(normalizeId(pageId), properties);
}

function todoNotePropertyName(properties = {}) {
  return ['備註', '筆記', '說明', '內容', '詳細內容'].find((name) => properties[name]?.type === 'rich_text') || '';
}

function addDateMessage(type, priority, name) {
  const meta = TODO_TYPES[type];
  const base = `action=add_date&type=${type}&priority=${priority}&name=${encodeURIComponent(name)}`;
  return textMessage(`${meta.emoji} ${name}\n優先級：${priority}\n\n要設定「預定作業日期」嗎？`, [
    quickPostback('📅 選擇日期', base, { type: 'datetimepicker', mode: 'date', initial: taipeiToday() }),
    quickPostback('不填日期', base),
    quickPostback('取消', `action=add_menu&type=${type}`),
  ]);
}

function addDescriptionMessage(type, priority, name, date = '') {
  const meta = TODO_TYPES[type];
  const prefix = type === 'buy' ? '新增待買說明' : '新增待辦說明';
  const datePart = date || '無';
  return textMessage(`${meta.emoji} ${name}\n優先級：${priority}\n預定作業日期：${date || '未設定'}\n\n要加上一段說明嗎？`, [
    quickPostback('⌨️ 填寫說明', 'action=noop', { inputOption: 'openKeyboard', fillInText: `${prefix}｜${priority}｜${datePart}｜${name}｜` }),
    quickPostback('略過，直接新增', `action=add_create&type=${type}&priority=${priority}&date=${encodeURIComponent(date)}&name=${encodeURIComponent(name)}`),
    quickPostback('取消', `action=add_menu&type=${type}`),
  ]);
}

async function addTodo(notion, config, type, priority, name, note = '', date = '') {
  const meta = TODO_TYPES[type];
  const properties = {
  ...
  if (/^\d{4}-\d{2}-\d{2}$/.test(date)) properties['預定作業日期'] = { date: { start: date } };
  let noteProperty = '';
  if (note && typeof notion.getDatabase === 'function') {
    const database = await notion.getDatabase(config.todoDbId);
    noteProperty = todoNotePropertyName(database.properties);
    if (noteProperty) properties[noteProperty] = { rich_text: [{ text: { content: note } }] };
  }
  const page = await notion.createPage(config.todoDbId, properties);
  if (note && !noteProperty && typeof notion.appendBlockChildren === 'function' && page?.id) {
    await notion.appendBlockChildren(page.id, [{ type: 'paragraph', paragraph: { rich_text: [{ type: 'text', text: { content: note } }] } }]);
  }
  return page;
}
...
async function getBills(notion, config) {
  ...
  return pages.map((page) => ({
    ...
    amountUpdatedDate: page.last_edited_time ? taipeiToday(new Date(page.last_edited_time)) : '',
...
function billStatus(item, today = taipeiToday()) {
  ...
  return `📅 ${item.dueDate.slice(5).replace('-', '/')}`;
}

function showsAmountUpdateDate(name = '') {
  return /信用卡|電信/.test(String(name));
}

function billsMessage(items, view = 'month', today = taipeiToday()) {
  const active = items.filter((item) => !item.paused);
  const monthEnd = `${today.slice(0, 8)}${new Date(Date.UTC(Number(today.slice(0, 4)), Number(today.slice(5, 7)), 0)).getUTCDate()}`;
  const visibleItems = view === 'all'
    ? [...active, ...items.filter((item) => item.paused)]
    : active.filter((item) => item.dueDate && item.dueDate <= monthEnd);
  const shown = visibleItems.slice(0, 10);
  const rows = shown.length ? shown.map((item) => {
    const contents = [
      { type: 'box', layout: 'vertical', flex: 4, contents: [
        ...
        { type: 'text', text: `${billStatus(item)}${money(item.price) ? ` ${money(item.price)}` : ''}${item.amountUpdatedDate && showsAmountUpdateDate(item.name) ? `（${item.amountUpdatedDate.slice(5).replace('-', '/')}更新）` : ''}${item.cancelling ? '・準備取消' : ''}`, size: 'xs', color: daysFromToday(item.dueDate || '9999-12-31') < 0 ? '#C2413B' : '#6B7280', wrap: true },
      ] },
      ...(item.paused ? [] : [{ type: 'button', style: 'link', height: 'sm', flex: 1, action: postbackAction('繳費', `action=paid_amount&id=${item.id}`) }]),
    ];
    return { type: 'box', layout: 'horizontal', alignItems: 'center', spacing: 'sm', contents };
  }) : [{ type: 'text', text: '目前沒有繳費項目 🎉', color: '#6B7280' }];
  const attention = active.filter((item) => daysFromToday(item.dueDate, today) <= 7).length;
  const isAll = view === 'all';
  const monthNumber = Number(today.slice(5, 7));
  return flexMessage(isAll ? `全部繳費項目：${items.length} 項` : `${monthNumber} 月繳費：${visibleItems.length} 項`, baseBubble(
    isAll ? '💳 全部繳費' : '💳 本月繳費',
    isAll
      ? `共 ${items.length} 項`
      : `${monthNumber} 月共 ${visibleItems.length} 項${attention ? `・${attention} 項需留意` : ''}`,
    rows,
    [
      button('✏️ 更新金額', 'action=update_amount_menu', 'primary'),
      button(isAll ? '← 只看本月' : '📋 查看全部', isAll ? 'action=bills' : 'action=bills_all'),
      button('🔄 重新整理', isAll ? 'action=bills_all' : 'action=bills'),
    ],
  ), mainQuickReplies());
}

function expiryMessage(items, view = 'attention') {
  const shown = items.slice(0, 10);
  const rows = shown.length ? shown.map((item) => {
    ...
    return { type: 'box', layout: 'vertical', spacing: 'xs', contents: [
      { type: 'text', text: truncate(item.name, 38), weight: 'bold', size: 'sm', wrap: true },
      { type: 'text', text: label, size: 'xs', color: item.daysRemaining < 0 ? '#C2413B' : '#6B7280' },
    ] };
  }) : [{
    type: 'text',
    text: view === 'all' ? '目前沒有未處理的效期品項。' : '目前沒有需要注意的效期品項。',
    color: '#6B7280', wrap: true,
  }];
  const overdueCount = items.filter((item) => item.daysRemaining < 0).length;
  const upcomingCount = items.filter((item) => item.daysRemaining >= 0 && item.daysRemaining <= 3).length;
  const isAll = view === 'all';
  const isOverdue = view === 'overdue';
  const title = isAll ? '⌛ 全部效期' : isOverdue ? '⌛ 已過期' : '⌛ 效期提醒';
  const subtitle = isAll
    ? `未處理共 ${items.length} 項`
    : isOverdue ? `共 ${overdueCount} 項` : `已過期 ${overdueCount} 項・3 天內 ${upcomingCount} 項`;
  const refreshAction = isAll ? 'action=expiry_all'
    : isOverdue ? 'action=expiry&overdueOnly=1' : 'action=expiry';
  return flexMessage(`${title}：${items.length} 項`, baseBubble(
    title, subtitle, rows,
    [
      button(isAll ? '← 只看需注意' : '查看全部', isAll ? 'action=expiry' : 'action=expiry_all', 'primary'),
      button(isOverdue ? '查看需注意' : '僅看已過期', isOverdue ? 'action=expiry' : 'action=expiry&overdueOnly=1'),
      button('重新整理', refreshAction),
    ],
  ), mainQuickReplies());
}

function updateAmountMenuMessage(items) {
  const active = items.filter((item) => !item.paused).slice(0, 10);
  if (!active.length) return textMessage('目前沒有可以更新金額的繳費項目。', [quickPostback('↩️ 返回繳費狀態', 'action=bills')]);
  return flexMessage('選擇要更新金額的項目', baseBubble(
    '✏️ 更新金額', '請選擇帳單，再輸入新的金額。', [],
    [
      ...active.map((item, index) => button(
        `${truncate(item.name, 16)}${money(item.price) ? `（目前 ${money(item.price)}）` : ''}`,
        'action=noop', index === 0 ? 'primary' : 'secondary',
        { inputOption: 'openKeyboard', fillInText: `更新金額 ${item.name} ` },
      )),
      button('↩️ 返回繳費狀態', 'action=bills'),
    ],
  ));
}
...
async function updateBillAmount(notion, config, name, amount) {
  const bill = await findBillByName(notion, config, name);
  if (!bill) return { found: false };
  const normalizedAmount = validAmount(amount);
  if (!normalizedAmount) return { found: true, updated: false };
  await notion.updatePage(bill.id, { '價格': { number: normalizedAmount } });
  return { found: true, updated: true, bill, amount: normalizedAmount, updatedDate: taipeiToday() };
}
...
function parseTextCommand(text) {
  ...
  const updateAmount = value.match(/^更新金額\s+(.+?)\s+([\d,]+(?:\.\d{1,2})?)$/);
  if (updateAmount) return { action: 'update_bill_amount', name: updateAmount[1].trim(), amount: validAmount(updateAmount[2]) };
  const addWithDateAndNote = value.match(/^新增(待辦|待買)說明｜(急|中|緩)｜(無|\d{4}-\d{2}-\d{2})｜([^｜]+)｜(.+)$/s);
  if (addWithDateAndNote) return {
    action: 'add_create', type: addWithDateAndNote[1] === '待買' ? 'buy' : 'todo', priority: addWithDateAndNote[2],
    date: addWithDateAndNote[3] === '無' ? '' : addWithDateAndNote[3],
    name: addWithDateAndNote[4].trim(), note: addWithDateAndNote[5].trim(),
  };
  const addWithNote = value.match(/^新增(待辦|待買)說明｜(急|中|緩)｜([^｜]+)｜(.+)$/s);
  if (addWithNote) return {
    action: 'add_create', type: addWithNote[1] === '待買' ? 'buy' : 'todo', priority: addWithNote[2],
    name: addWithNote[3].trim(), note: addWithNote[4].trim(),
  };
  ...
  if (/^(效期|快過期)$/.test(value)) return { action: 'expiry', overdueOnly: false };
  if (value === '過期') return { action: 'expiry', overdueOnly: true };
...
async function handleCommand(command, notion, config) {
  ...
  if (command.action === 'calendar_range' || command.action === 'calendar_date') {
    ...
    if (command.action === 'calendar_date') {
      start = command.date; end = addDays(start, 1); label = start.slice(5).replace('-', '/');
    } else {
  ...
  if ((command.action === 'todo_detail' || command.action === 'checklist') && TODO_TYPES[command.type] && validPageId(command.id)) {
    return [checklistMessage(command.type, await getTodoDetail(notion, command.id), command.page)];
  }
  if (command.action === 'edit_reminder' && TODO_TYPES[command.type] && validPageId(command.id)) {
    return [editReminderMessage(command.type, await getTodoDetail(notion, command.id))];
  }
  if (command.action === 'edit_priority' && TODO_TYPES[command.type] && validPageId(command.id)
      && ['急', '中', '緩'].includes(command.priority)) {
    await updateTodoReminder(notion, command.id, { priority: command.priority });
    return [editReminderMessage(command.type, await getTodoDetail(notion, command.id))];
  }
  if (command.action === 'edit_date' && TODO_TYPES[command.type] && validPageId(command.id)) {
    const date = /^\d{4}-\d{2}-\d{2}$/.test(command.date || '') ? command.date : '';
    if (!date && command.clear !== '1') return [editReminderMessage(command.type, await getTodoDetail(notion, command.id))];
    await updateTodoReminder(notion, command.id, { date, clearDate: command.clear === '1' });
    return [editReminderMessage(command.type, await getTodoDetail(notion, command.id))];
  }
  if (command.action === 'checklist_toggle' && TODO_TYPES[command.type]
      && validPageId(command.id) && validPageId(command.block)) {
    await toggleChecklistItem(notion, command.id, command.block, command.checked === '1');
    // LINE 無法修改已送出的訊息；靜默同步避免每勾一項就堆疊一張新卡片。
    // 使用者完成數項後，可按卡片下方的「重新整理清單」查看最新狀態。
    return [];
  }
  ...
  if (command.action === 'add' && TODO_TYPES[command.type]) {
    ...
    return [addDateMessage(command.type, command.priority, name)];
  }
  if (command.action === 'add_date' && TODO_TYPES[command.type] && ['急', '中', '緩'].includes(command.priority)) {
    const name = truncate(command.name, 120);
    const date = /^\d{4}-\d{2}-\d{2}$/.test(command.date || '') ? command.date : '';
    if (!name) return [addMenuMessage(command.type)];
    return [addDescriptionMessage(command.type, command.priority, name, date)];
  }
  if (command.action === 'add_create' && TODO_TYPES[command.type] && ['急', '中', '緩'].includes(command.priority)) {
    const name = truncate(command.name, 120);
    const note = truncate(command.note || '', 1000);
    if (!name) return [addMenuMessage(command.type)];
    const date = /^\d{4}-\d{2}-\d{2}$/.test(command.date || '') ? command.date : '';
    await addTodo(notion, config, command.type, command.priority, name, note, date);
    const meta = TODO_TYPES[command.type];
    return [textMessage(`${meta.emoji} 已新增：${name}\n優先級：${command.priority}${date ? `\n預定作業日期：${date}` : ''}${note ? `\n說明：${note}` : ''}`, [
  ...
  if (command.action === 'expiry') {
    const items = await getExpiryItems(notion, config.expiryDbId, taipeiToday(), {
      overdueOnly: Boolean(command.overdueOnly),
    });
    return [expiryMessage(items, command.overdueOnly ? 'overdue' : 'attention')];
  }
  if (command.action === 'expiry_all') {
    const items = await getExpiryItems(notion, config.expiryDbId, taipeiToday(), { includeAll: true });
    return [expiryMessage(items, 'all')];
  }
  if (command.action === 'bills_all') return [billsMessage(await getBills(notion, config), 'all')];
  if (command.action === 'update_amount_menu') return [updateAmountMenuMessage(await getBills(notion, config))];
  if (command.action === 'update_bill_amount') {
    const result = await updateBillAmount(notion, config, command.name, command.amount);
    if (!result.found) return [textMessage(`找不到「${truncate(command.name, 50)}」這個繳費項目。\n請確認名稱與 Notion 完全相同。`)];
    if (!result.updated) return [textMessage('金額格式不正確，請輸入大於 0 的數字。\n例如：更新金額 台灣大哥大 1380')];
    return [textMessage(`已更新金額\n${result.bill.name}：${money(result.amount)}\n更新：${result.updatedDate.slice(5).replace('-', '/')}`)];
  }
...
module.exports = {
  ...
  todoListMessage, todoDetailMessage, checklistMessage, getChecklist, toggleChecklistItem,
  billsMessage, billStatus, showsAmountUpdateDate, updateAmountMenuMessage, paidAmountMessage, calendarMenuMessage, calendarAddMenuMessage,
  calendarStartMessage, calendarLabelText, quickAddMessage, calendarEventsMessage,
  getExpiryItems, formatExpiryBlock, expiryMessage,
  handleCommand, markBillPaid, updateBillAmount,
};
