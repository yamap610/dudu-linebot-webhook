...
function calendarEventsMessage(label, events) {
  ...
  const activityEvents = events.filter((event) => {
    const title = event.summary || '';
    const location = calendarLocation(event.eventLabelName, event.colorId, title);
    return !(location && title.startsWith(location));
  });
  const lines = activityEvents.slice(0, 30).map((event) => {
  ...
  const showLocationDate = label.includes('週') || uniqueLocationEntries.length > 1;
  const locationLines = uniqueLocationEntries.map(({ date, location }) => `${showLocationDate ? `${date}｜` : ''}📍 ${location}`);
  const content = [...locationLines, ...lines];
  return textMessage(`📅 ${label}\n\n${content.join('\n')}${activityEvents.length > 30 ? `\n\n另有 ${activityEvents.length - 30} 筆未顯示` : ''}`, mainQuickReplies());
}
