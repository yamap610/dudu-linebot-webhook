...
function checklistMessage(type, item, requestedPage = 1) {
  ...
  const rows = shown.map((entry, index) => ({
    ...
    contents: [
      ...
      { type: 'button', style: entry.checked ? 'link' : 'primary', height: 'sm', flex: 2,
        ...(entry.checked ? {} : { color: '#E8A0A8' }),
  ...
  return flexMessage(`${meta.label}內容清單`, baseBubble(
    ...
    [...(item.note ? [{ type: 'text', text: `說明：${truncate(item.note, 180)}`, wrap: true, size: 'xs', color: '#6B7280', margin: 'md' }] : []), ...rows],
    footer,
  ));
}
