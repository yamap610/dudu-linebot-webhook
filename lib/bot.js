...
async function handleCommand(command, notion, config) {
  ...
  if (command.action === 'calendar_add') {
    ...
    const endDate = String(command.end || '').slice(5, 10).replace('-', '/');
    const when = detailed
      ? (allDay ? `${startDate} 整天` : (command.start.slice(0, 10) === command.end.slice(0, 10)
        ? `${startDate} ${command.start.slice(11)}–${command.end.slice(11)}`
        : `${startDate} ${command.start.slice(11)}–${endDate} ${command.end.slice(11)}`))
