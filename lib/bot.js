...
const { createCalendarClient, addDays, weekRange, eventDate, eventTime } = require('./calendar');
...
function button(label, data, style = 'secondary') {
  return { type: 'button', style, height: 'sm', action: postbackAction(label, data) };
}
...
function calendarAddMenuMessage() {
  return textMessage('📅 新增 Google 行程\n先選擇日期與時間，再輸入行程名稱。', [
    quickPostback('🕘 選擇日期時間', 'action=calendar_add_pick', { type: 'datetimepicker', mode: 'datetime', initial: `${taipeiToday()}T09:00` }),
    quickPostback('☀️ 新增整天行程', 'action=calendar_add_day', { type: 'datetimepicker', mode: 'date', initial: taipeiToday() }),
    quickPostback('↩️ 回快速新增', 'action=quick_add'),
  ]);
}

function calendarEventsMessage(label, events) {
  if (!events.length) return textMessage(`📅 ${label}\n目前沒有行程。`, mainQuickReplies());
  const lines = events.slice(0, 30).map((event) => `${eventDate(event).replace(/-/g, '/')}｜${eventTime(event)}｜${truncate(event.summary || '（未命名）', 50)}`);
...
function todoListMessage(type, items) {
  ...
  const rows = shown.length ? shown.map((item) => ({
    ...
    contents: [
      { type: 'text', text: `${item.priority === '急' ? '🔥' : item.priority === '緩' ? '🌿' : '⭐'} ${truncate(item.name, 38)}`, wrap: true, size: 'sm', flex: 4 },
      { type: 'button', style: 'link', height: 'sm', flex: 1, action: postbackAction('完成', `action=complete_confirm&type=${type}&id=${item.id}`) },
    ],
  })) : [{ type: 'text', text: '目前沒有未完成項目 🎉', color: '#6B7280', wrap: true }];
  const subtitle = items.length > 10 ? `共 ${items.length} 項，先顯示前 10 項` : `共 ${items.length} 項未完成`;
...
function confirmationMessage(title, detail, confirmData, cancelData) {
  return flexMessage(title, baseBubble('請確認', detail, [], [
    button('確認', confirmData, 'primary'), button('取消', cancelData),
  ]));
}

async function searchWiki(notion, config, { keyword, category }) {
  const pages = await notion.queryAll(config.wikiDbId, {
    sorts: [{ property: '精選', direction: 'descending' }, { timestamp: 'last_edited_time', direction: 'descending' }],
  });
...
function billsMessage(items) {
  ...
  const rows = shown.length ? shown.map((item) => {
    const contents = [
      ...
      ...(item.paused ? [] : [{ type: 'button', style: 'link', height: 'sm', flex: 1, action: postbackAction('已繳', `action=paid_confirm&id=${item.id}`) }]),
    ];
...
async function markBillPaid(notion, config, pageId, now = new Date()) {
...
async function handleCommand(command, notion, config) {
  ...
  if (command.action === 'calendar_add_menu') return [calendarAddMenuMessage()];
  if (command.action === 'calendar_add_pick' || command.action === 'calendar_add_day') {
    const dateTime = command.datetime || command.date || '';
    const date = dateTime.slice(0, 10); const time = command.action === 'calendar_add_day' ? '全天' : dateTime.slice(11, 16);
    return [textMessage(`✏️ 請輸入行程名稱\n\n格式：新增行程 ${date} ${time} 行程名稱`, [
      quickPostback('⌨️ 輸入名稱', 'action=noop', { inputOption: 'openKeyboard', fillInText: `新增行程 ${date} ${time} ` }),
      quickPostback('取消', 'action=calendar_menu'),
    ])];
  }
  if (command.action === 'calendar_add') {
    const name = truncate(command.name, 120);
    if (!name || !/^\d{4}-\d{2}-\d{2}$/.test(command.date)) return [calendarAddMenuMessage()];
    const calendar = createCalendarClient(config);
    await calendar.createEvent({ title: name, date: command.date, time: command.time });
    return [textMessage(`✅ 已新增 Google 行程\n${command.date} ${command.time || '整天'}｜${name}`, [
  ...
  if (command.action === 'complete_confirm' && TODO_TYPES[command.type] && validPageId(command.id)) {
    return [confirmationMessage('完成項目', '確定要把這一項標記為完成嗎？', `action=complete&type=${command.type}&id=${normalizeId(command.id)}`, `action=list&type=${command.type}`)];
  }
  ...
  if (command.action === 'paid_confirm' && validPageId(command.id)) {
    ...
    if (!bill) throw new Error('找不到這筆繳費項目');
    return [confirmationMessage('標記已繳', `${bill.name}\n${bill.dueDate}${money(bill.price) ? `・${money(bill.price)}` : ''}\n\n確認後會新增一筆歷史開銷紀錄。`, `action=paid&id=${bill.id}`, 'action=bills')];
  }
  if (command.action === 'paid') {
    const result = await markBillPaid(notion, config, command.id);
    const message = result.duplicate
      ? `ℹ️ ${result.bill.name} 今天已有繳費紀錄，未重複新增。`
      : `✅ ${result.bill.name} 已標記為已繳，Notion 已同步更新。`;
...
module.exports = {
  DB_DEFAULTS, TODO_TYPES, WIKI_CATEGORIES, getConfig, validateConfig, createNotionClient,
  richText, formulaDate, normalizeId, validPageId, taipeiToday, daysFromToday,
  truncate, parseTextCommand, parsePostback, mainMenuMessage, wikiMenuMessage,
  todoListMessage, billsMessage, calendarMenuMessage, quickAddMessage, calendarEventsMessage,
