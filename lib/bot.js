...
function button(label, data, style = 'secondary', extra = {}) {
  return { type: 'button', style, height: 'sm', action: postbackAction(label, data, extra) };
}
...
function todoListMessage(type, items) {
  ...
  const rows = shown.length ? shown.map((item) => ({
    ...
    contents: [
      ...
      ...(item.note ? [{ type: 'text', text: `📝 ${truncate(item.note, 70)}`, wrap: true, size: 'xs', color: '#6B7280' }] : []),
...
module.exports = {
  ...
  todoListMessage, todoDetailMessage, billsMessage, paidAmountMessage, calendarMenuMessage, quickAddMessage, calendarEventsMessage,
